...
public class SPLFileAction extends AnAction {
    ...
    public void actionPerformed(@NotNull AnActionEvent e) {
        ...
        if (selectedDirectory != null) {
            ...
            String fileName = Messages.showInputDialog(project, "Enter new file name:", "New SPL File", Messages.getQuestionIcon());

            if (fileName != null && !fileName.trim().isEmpty()) {
                // Trim whitespace and check for ".spl" extension
                String trimmedFileName = fileName.trim();
                if (!trimmedFileName.toLowerCase().endsWith(".spl")) {
                    trimmedFileName += ".spl"; // Append .spl if not already present
                }
                ...
                ApplicationManager.getApplication().runWriteAction(() -> {
                    try {
                        // Use SPLFileAction.this to refer to the outer class
